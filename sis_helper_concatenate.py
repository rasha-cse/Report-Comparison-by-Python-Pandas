...
therap_input_file_for_Concatenate = 'therap_all_scores_dates_6_nov_2017_production(2).xlsx'
date = '6_nov_2017'

input_therap_path = r"C:\Users\rasha\PycharmProjects\SIS_Comparison_Report\input" + "\\"  + therap_input_file_for_Concatenate
input_sis_online_path = r"C:\Users\rasha\PycharmProjects\SIS_Comparison_Report\input\selected_columns_sis_online_for_concatenate_" + date + ".xlsx"
output_therap_path = r"C:\Users\rasha\PycharmProjects\SIS_Comparison_Report\output\concatenated_therap_" + date + ".xlsx"
output_sis_online_path = r"C:\Users\rasha\PycharmProjects\SIS_Comparison_Report\output\concatenated_sis_online_" + date + ".xlsx"

therap_prod = pd.read_excel(input_therap_path, sheetname='Export Worksheet', na_values=['NA'], converters={'SIS_TRACKING_NUM': lambda x: str(x), 'HOME_LIVING_RAW': lambda x: str(x), 'HOME_LIVING_STANDARD': lambda x: str(x), 'HOME_LIVING_PERCENTILE': lambda x: str(x), 'COMMUNITY_LIVING_RAW': lambda x: str(x), 'COMMUNITY_LIVING_STANDARD': lambda x: str(x), 'COMMUNITY_LIVING_PERCENTILE': lambda x: str(x), 'LIFELONG_LEARNING_RAW': lambda x: str(x), 'LIFELONG_LEARNING_STANDARD': lambda x: str(x), 'LIFELONG_LEARNING_PERCENTILE': lambda x: str(x), 'EMPLOYMENT_RAW': lambda x: str(x), 'EMPLOYMENT_STANDARD': lambda x: str(x), 'EMPLOYMENT_PERCENTILE': lambda x: str(x), 'HEALTH_SAFETY_RAW': lambda x: str(x), 'HEALTH_SAFETY_STANDARD': lambda x: str(x), 'HEALTH_SAFETY_PERCENTILE': lambda x: str(x), 'SOCIAL_RAW': lambda x: str(x), 'SOCIAL_STANDARD': lambda x: str(x), 'SOCIAL_PERCENTILE': lambda x: str(x), 'TOTAL_SCORE_NEEDED_INDEX': lambda x: str(x), 'EXCEPTION_MEDICAL_TOTAL': lambda x: str(x), 'EXCEPTION_BEHAVE_TOTAL': lambda x: str(x) })
...
sis_online['Tracking Number'] = sis_online['Tracking Number'].fillna(0)  #give explanation
